# mpv 和 mpv.net 开箱即用配置文件&精美 UI 主题

## 简要说明

- 使用原生 mpv.net 播放器
- 集成进度条缩略图预览：[Thumbfast](https://github.com/po5/thumbfast)
- 集成漂亮的 UI 无边框设计：[ModernZ](https://github.com/Samillion/ModernZ)
- 快捷键优化 `Ctrl + i` 查看和设置所有快捷键

## 使用方法

- 先安装 mpv.net 播放器：[https://github.com/mpvnet-player/mpv.net/releases](https://github.com/mpvnet-player/mpv.net/releases)
- [🎁 下载本仓库所有文件](https://github.com/akFace/mpv.net.config/releases)，或者`git clone https://github.com/akFace/mpv.net.config.git`
- 如图所示，右键>配置>打开配置文件夹或者`Ctrl + f`快捷键打开配置文件夹

  ![image](https://raw.githubusercontent.com/akFace/mpv.net.config/master/preview/Snipaste_2026-03-16_20-34-06.jpg)

- 将所有文件复制到配置文件夹，重启播放器即可
- 若你使用的并非 mpv.net 播放器，请修改`script-opts/thumbfast.conf`目录中的`mpv_path=mpvnet`改为`mpv_path=mpv`或者播放器安装目录可执行文件 例如：`mpv_path=eC:\Program Files\mpv.net\mpvnet.exe`

## 预览效果图：

- 无边框设计，拖动进度条预览图
  ![image](https://raw.githubusercontent.com/akFace/mpv.net.config/master/preview/Snipaste_2026-03-16_20-32-59.jpg)
  ![image](https://raw.githubusercontent.com/akFace/mpv.net.config/master/preview/Snipaste_2026-03-16_20-34-33.jpg)
